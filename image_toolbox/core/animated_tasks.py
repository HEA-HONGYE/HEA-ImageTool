...
class AnimatedMediaTask(QRunnable):
    ...
    def _write_task_json(self, task_dir: Path, metadata: AnimatedTaskMetadata) -> None:
        (task_dir / "task.json").write_text(json.dumps(asdict(metadata), ensure_ascii=False, indent=2), encoding="utf-8")

    def _extract_frames(self, source: Path, frame_dir: Path, info: AnimatedMediaInfo, file_index: int, total_files: int) -> None:
        frame_dir.mkdir(parents=True, exist_ok=True)
        self.signals.log.emit(f"拆帧：{source.name}，共 {info.frame_count} 帧")
        with Image.open(source) as image:
    ...
    def _process_frames(self, frames_in: Path, frames_out: Path, file_index: int, total_files: int) -> Path:
        ...
        for index, frame_path in enumerate(frames, start=1):
            ...
            try:
                self._upscale_runner.run(frame_path, output_path, self.settings.upscale_settings, self._cancel_event, self.signals.log.emit, self._debug, on_progress)
            finally:
    ...
    def _compose(self, frames_dir: Path, output_path: Path, info: AnimatedMediaInfo, file_index: int, total_files: int) -> None:
        ...
        self.signals.log.emit(f"合成动态图片：{output_path.name}，格式：{fmt.upper()}，帧数：{len(frames)}")
        try:
            if fmt == "gif":
                ...
                if info.has_alpha:
                    self.signals.log.emit("透明通道警告：GIF 仅支持 1-bit 透明，可能有边缘损失。")
            elif fmt == "webp":
    ...
    def _process_one(self, source: Path, output_path: Path, task_dir: Path, file_index: int, total_files: int) -> None:
        ...
        self._write_task_json(task_dir, metadata)
